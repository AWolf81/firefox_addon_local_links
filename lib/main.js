...
function onAttach( worker ) {
    ...
    */
    ...
    function onPrefChangeIcon( prefName ) { // Re-name to onEnableLinkChange
        var newEmitObj = {};
    ...
    function onPrefChangeSamba( prefName ) {
        console.log('prefchanged',  prefs.options[ prefName ], prefName, prefName == 'sambaConfigured');
        // on sambaConfigured changed to true
        if (  prefs.options[ prefName ] ) {
            worker.port.emit( "undelegateSambaClick" ); // now firefox handles smb/afp
        }
    }

    prefs.addPrefChangeHandler( "enableLinkIcons", onPrefChangeIcon );
    prefs.addPrefChangeHandler( "sambaConfigured", onPrefChangeSamba );
}
...
function createMod() {
    ...
    mod = pageMod.PageMod( {
        ...
        contentScriptOptions: {
            ...
            sambaConfigured: prefs.options.sambaConfigured // hidden option (not visible in prefs pane)
        },
        contentScriptFile: [
